...
const step = (n, title, body) => `<table role="presentation" style="border-collapse:collapse;margin:0 0 16px"><tr>
  <td style="vertical-align:top;padding-right:16px"><div style="width:32px;height:32px;border-radius:50%;background:#f6f8fb;border:1px solid #c9a437;color:#8a6d10;font-weight:bold;font-size:14px;text-align:center;line-height:32px;font-family:Georgia,serif">${n}</div></td>
  <td style="vertical-align:top"><p style="margin:4px 0 3px;color:#0a1628;font-size:15px;font-weight:bold">${title}</p><p style="margin:0;color:#46505f;font-size:14px;line-height:1.6">${body}</p></td></tr></table>`;
...
export default async function handler(req, res) {
  ...
  const rows = [["Navn", navn], ["E-mail", email], ["Telefon", telefon], ["Pakke", pakke], ["Område", omraade], ["Forløb", forloeb], ["Besked", besked]]
    .filter(([, v]) => v)
    .map(([k, v], i, arr) => `<tr><td style="color:#697585;font-size:13px;padding:11px 14px 11px 18px;vertical-align:top;white-space:nowrap;${i < arr.length - 1 ? "border-bottom:1px solid #eef1f4;" : ""}">${k}</td><td style="color:#0a1628;font-size:14px;padding:11px 18px 11px 0;${i < arr.length - 1 ? "border-bottom:1px solid #eef1f4;" : ""}">${esc(v)}</td></tr>`)
    .join("");
  ...
  try {
    await sendMail({
      ...
      html: shell("Til bekr\u00e6ftelse", `<h1 style="color:#0a1628;font-size:23px;margin:0 0 18px;font-family:Georgia,serif;font-weight:normal">Ny booking venter p\u00e5 en tid</h1>
        <table role="presentation" style="border-collapse:collapse;width:100%;margin:0 0 24px;background:#f6f8fb;border:1px solid #eef1f4;border-radius:12px">${rows}</table>
        <div style="text-align:center;margin:0 0 6px"><a href="${link}" style="display:inline-block;background:#eef2f7;color:#ffffff;font-weight:bold;text-decoration:none;border-radius:999px;padding:15px 34px;font-size:15px">Bekr\u00e6ft eller foresl\u00e5 tid</a></div>
        <p style="color:#697585;font-size:13px;line-height:1.6;margin:14px 0 0;text-align:center">Linket \u00e5bner en side, hvor du v\u00e6lger den konkrete tid.<br>Kunden f\u00e5r automatisk besked og betalingslink.</p>`),
    });
    ...
    await sendMail({
      ...
      html: shell("Din booking", `<h1 style="color:#0a1628;font-size:24px;margin:0 0 14px;font-family:Georgia,serif;font-weight:normal">Tak${navn ? ", " + esc(navn.split(" ")[0]) : ""} \u2014 vi er i gang</h1>
        <p style="color:#46505f;font-size:15px;line-height:1.7;margin:0 0 24px">Vi har modtaget dit \u00f8nskede forl\u00f8b. Her er, hvad der sker nu:</p>
        ${pakke || omraade ? `<div style="background:#f6f8fb;border:1px solid #eef1f4;border-left:3px solid #c9a437;border-radius:10px;padding:14px 18px;margin:0 0 26px">${pakke ? `<p style="margin:0;color:#0a1628;font-size:15px;font-weight:bold">${esc(pakke)}</p>` : ""}${omraade ? `<p style="margin:${pakke ? "4px" : "0"} 0 0;color:#697585;font-size:13px">${esc(omraade)}</p>` : ""}</div>` : ""}
        ${step(1, "Vi koordinerer din tid", "Vi finder en konkret tid hos laboratoriet i dit omr\u00e5de \u2014 du h\u00f8rer fra os inden for <strong style='color:#0a1628'>1 arbejdsdag</strong>.")}
